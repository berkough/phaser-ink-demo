...
class Story extends FlowBase_1.FlowBase {
    constructor(toplevelObjects, isInclude = false) {
        ...
        this.FlattenContainersIn = (container) => {
            ...
            if (container.content) {
                for (const c of container.content) {
                    const innerContainer = TypeAssertion_1.asOrNull(c, Container_1.Container);
                    if (innerContainer) {
            ...
            if (container.namedContent) {
                for (const [, value] of container.namedContent) {
                    const namedInnerContainer = TypeAssertion_1.asOrNull(value, Container_1.Container);
        ...
        this.TryFlattenContainer = (container) => {
            ...
            const parentContainer = TypeAssertion_1.asOrNull(container.parent, Container_1.Container);
            if (parentContainer) {
        ...
        this.CheckForNamingCollisions = (obj, identifier, symbolType, typeNameOverride = "") => {
            ...
            const knotOrFunction = TypeAssertion_1.asOrNull(maybeKnotOrFunction, FlowBase_1.FlowBase);
            if (knotOrFunction &&
            ...
            if (symbolType !== SymbolType_1.SymbolType.Arg) {
                let flow = TypeAssertion_1.asOrNull(obj, FlowBase_1.FlowBase);
                if (!flow) {
                    flow = ClosestFlowBase_1.ClosestFlowBase(obj);
                }
