...
class Weave extends Object_1.ParsedObject {
    constructor(cont, indentIndex = -1) {
            ...
            while (contentIdx < this.content.length) {
                ...
                if (obj instanceof Choice_1.Choice || obj instanceof Gather_1.Gather) {
                    ...
                    if (weaveIndentIdx > this.baseIndentIndex) {
                        ...
                        while (contentIdx < this.content.length) {
                            const innerWeaveObj = TypeAssertion_1.asOrNull(this.content[contentIdx], Choice_1.Choice) ||
                                TypeAssertion_1.asOrNull(this.content[contentIdx], Gather_1.Gather);
        ...
        this.AddRuntimeForWeavePoint = (weavePoint) => {
            ...
            if (this.WeavePointHasLooseEnd(weavePoint)) {
                ...
                const looseChoice = TypeAssertion_1.asOrNull(weavePoint, Choice_1.Choice);
                if (looseChoice) {
        ...
        this.PassLooseEndsToAncestors = () => {
            ...
            for (let ancestor = this.parent; ancestor !== null; ancestor = ancestor.parent) {
                // Found ancestor?
                const weaveAncestor = TypeAssertion_1.asOrNull(ancestor, Weave);
                if (weaveAncestor) {
        ...
        this.IsGlobalDeclaration = (obj) => {
            const varAss = TypeAssertion_1.asOrNull(obj, VariableAssignment_1.VariableAssignment);
            if (varAss && varAss.isGlobalDeclaration && varAss.isDeclaration) {
                return true;
            }
            const constDecl = TypeAssertion_1.asOrNull(obj, ConstantDeclaration_1.ConstantDeclaration);
            if (constDecl) {
                return true;
            }
            return false;
        ...
        this.ContentThatFollowsWeavePoint = (weavePoint) => {
            ...
            const parentWeave = TypeAssertion_1.asOrNull(obj.parent, Weave);
            if (parentWeave === null) {
        ...
        this.BadNestedTerminationHandler = (terminatingObj) => {
            ...
            for (let ancestor = terminatingObj.parent; ancestor !== null; ancestor = ancestor.parent) {
                if (ancestor instanceof Sequence_1.Sequence || ancestor instanceof Conditional_1.Conditional) {
                    conditional = TypeAssertion_1.asOrNull(ancestor, Conditional_1.Conditional);
                    break;
                }
            }
        ...
        this.WeavePointHasLooseEnd = (weavePoint) => {
            ...
            for (let ii = weavePoint.content.length - 1; ii >= 0; --ii) {
                let innerDivert = TypeAssertion_1.asOrNull(weavePoint.content[ii], Divert_1.Divert);
                if (innerDivert) {
        ...
        this.CheckForWeavePointNamingCollisions = () => {
            ...
            for (const obj of this.ancestry) {
                const flow = TypeAssertion_1.asOrNull(obj, FlowBase_1.FlowBase);
                if (flow) {
    ...
    get lastParsedSignificantObject() {
        ...
        for (let ii = this.content.length - 1; ii >= 0; --ii) {
            ...
            let lastText = TypeAssertion_1.asOrNull(lastObject, Text_1.Text);
        ...
        const lastWeave = TypeAssertion_1.asOrNull(lastObject, Weave);
        if (lastWeave) {
